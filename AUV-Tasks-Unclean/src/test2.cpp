    ...
    //cout<<angle<<endl;
        if(abs(90.0-angle) < line_parameter && min(l[1],l[3])>120)//&&((l[1]>480-reflection_deleting_parameter)||(l[3]>480-reflection_deleting_parameter)))
              ...
              //line( src, Point(xa0, xa1), Point(xa2, xa3), Scalar(0,0,255), 4, CV_AA);
						...
						if(b==-1 && abs(xa0-l[0])>150 && (abs(xa1-xa3)-abs(l[1]-l[3]))<50)
						{
              int p,q;
              p=min(xa1,xa3);
              q=min(l[1],l[3]);
              ...
              if(c==-1)
              {
                ...
                if(b!=-1)
                {
                  ...
                  if(abs(l[0]-xb0)<0.45*abs(xa0-xb0) && abs(l[0]-xb0)>0.32*abs(xa0-xb0) && 0.35*length_b<abs(l[1]-l[3]) && abs(l[1]-l[3])<0.55*length_b)
                  {
                    int p,q;
                    p=min(xb1,xb3);
                    q=min(l[1],l[3]);
                  ...
                  else if(abs(l[0]-xa0)<0.45*abs(xa0-xb0) && abs(l[0]-xa0)>0.32*abs(xa0-xb0) && 0.34*length_a<abs(l[1]-l[3]) && abs(l[1]-l[3])<0.53*length_a)
                  {
                    int p,q;
                    p=min(xa1,xa3);
                    q=min(l[1],l[3]);
...
image_transport::Subscriber sub=it.subscribe("/front_camera/image_rect_color",10,call);
ros::spin();
}
